...
The dot is an exception to the ease-and-dwell motion: the reader's animation-frame clock drives a linear sweep from the leading word edge to the next word's leading edge (or the last word's trailing edge on a line). Wrapped fragments divide that word's time by rendered width. A maximum of 16 small trail particles fade over 180ms; line returns, pauses, reduced motion, settings changes, and disposal clear them. No second reading clock is created.
...
Cursor shape (hand/dot/arrow), size (12–40), stroke thickness (1–4), a six-color allowlist, and group size (1–4) are sanitized with defaults when older preferences are loaded. SVG viewBoxes preserve shape proportions. `walkthrough.ts` owns a disposable, interactive practice session inside the shared controls: choose a sample word, adjust practice speed, then play/pause. It has its own demo-only frame loop, stops on dismissal/hidden documents/disposal, and never sends practice settings or playback to the page. Dismissal synchronizes through local storage; the help button resets the tutorial for replay.
